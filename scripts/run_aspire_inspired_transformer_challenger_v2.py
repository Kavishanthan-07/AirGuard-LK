...
def main():
    ...
    # Save core artifacts first so a non-essential table export cannot
    # prevent the primary JSON/model from being preserved.
    ...
    # Prediction tables are small enough for CSV and this avoids requiring
    # optional parquet engines such as pyarrow / fastparquet.
    pred3.to_csv(args.out_dir / "F03_predictions.csv", index=False)
    pred4.to_csv(args.out_dir / "F04_predictions.csv", index=False)
